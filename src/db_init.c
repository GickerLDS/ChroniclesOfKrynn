...
void init_core_player_tables(void)
{
  ...
  const char *create_player_save_objs = "CREATE TABLE IF NOT EXISTS player_save_objs ("
                                        "id INT AUTO_INCREMENT PRIMARY KEY, "
...
void init_wilderness_resource_tables(void)
{
  ...
  const char *create_resource_types = "CREATE TABLE IF NOT EXISTS resource_types ("
  ...
  const char *create_ecosystem_health =
      ...
      "health_state ENUM('pristine', 'healthy', 'degraded', 'critical', 'devastated') DEFAULT "
      "'healthy', "
  ...
  const char *create_material_qualities = "CREATE TABLE IF NOT EXISTS material_qualities ("
...
void init_region_system_tables(void)
{
  ...
  const char *create_region_data = "CREATE TABLE IF NOT EXISTS region_data ("
  ...
  const char *create_path_data = "CREATE TABLE IF NOT EXISTS path_data ("
                                 "vnum INT PRIMARY KEY, "
  ...
  const char *create_path_types = "CREATE TABLE IF NOT EXISTS path_types ("
  ...
  const char *create_region_index = "CREATE TABLE IF NOT EXISTS region_index ("
                                    "vnum INT PRIMARY KEY, "
                                    "zone_vnum INT NOT NULL, "
  ...
  const char *create_path_index = "CREATE TABLE IF NOT EXISTS path_index ("
                                  "vnum INT PRIMARY KEY, "
                                  "zone_vnum INT NOT NULL, "
  ...
  const char *add_region_description_fields =
      ...
      "ADD COLUMN IF NOT EXISTS last_description_update TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON "
      "UPDATE CURRENT_TIMESTAMP, "
      "ADD COLUMN IF NOT EXISTS description_style ENUM('poetic', 'practical', 'mysterious', "
      "'dramatic', 'pastoral') DEFAULT 'poetic', "
      "ADD COLUMN IF NOT EXISTS description_length ENUM('brief', 'moderate', 'detailed', "
      "'extensive') DEFAULT 'moderate', "
  ...
  const char *add_region_description_indexes =
      "CREATE INDEX IF NOT EXISTS idx_region_has_description ON region_data "
      "(region_description(100)); "
      "CREATE INDEX IF NOT EXISTS idx_region_description_approved ON region_data (is_approved, "
      "requires_review); "
      "CREATE INDEX IF NOT EXISTS idx_region_description_quality ON region_data "
...
void init_region_hints_tables(void)
{
  ...
  const char *create_region_profiles =
      ...
      "description_style ENUM('poetic', 'practical', 'mysterious', 'dramatic', 'pastoral') DEFAULT "
      "'poetic', "
  ...
  const char *create_description_templates = "CREATE TABLE IF NOT EXISTS description_templates ("
                                             "id INT AUTO_INCREMENT PRIMARY KEY, "
                                             "region_vnum INT NOT NULL, "
                                             "template_type ENUM('intro', 'weather_overlay', "
                                             "'resource_state', 'time_transition') NOT NULL, "
  ...
  const char *create_hint_analytics_view = "CREATE OR REPLACE VIEW hint_analytics AS "
                                           "SELECT "
...
void init_ai_service_tables(void)
{
  ...
  const char *create_ai_cache = "CREATE TABLE IF NOT EXISTS ai_cache ("
  ...
  const char *create_ai_npc_personalities =
      ...
      "personality TEXT COMMENT 'JSON object with personality traits, background, speech "
      "patterns', "
...
void init_vessel_system_tables(void)
{
  ...
  const char *create_ship_room_templates = "CREATE TABLE IF NOT EXISTS ship_room_templates ("
...
static void create_vessel_procedures(void)
{
  ...
  const char *create_cleanup_proc = "CREATE PROCEDURE cleanup_orphaned_dockings()\n"
                                    "BEGIN\n"
                                    "    UPDATE ship_docking\n"
...
int apply_migration(int version, const char *description, const char *sql)
{
  ...
  snprintf(query, sizeof(query),
           "INSERT INTO schema_migrations (version, description) VALUES (%d, '%s')", version,
           description);
...
void init_help_system_tables(void)
{
  ...
  const char *add_composite_index = "ALTER TABLE help_keywords ADD INDEX IF NOT EXISTS "
                                    "idx_help_keywords_composite (help_tag, keyword)";
...
void create_database_procedures(void)
{
  ...
  const char *create_bresenham_segment = "CREATE OR REPLACE PROCEDURE bresenham_segment("
