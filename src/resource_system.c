...
const char *get_material_applications(int category)
{
  ...
  {
  case RESOURCE_HERBS:
    return "Alchemy potions, healing items";
  case RESOURCE_CRYSTAL:
    return "Magical items, enchanting";
  case RESOURCE_MINERALS:
    return "Weapons, armor, tools";
  case RESOURCE_WOOD:
    return "Crafted items, magical staves";
  case RESOURCE_VEGETATION:
    return "Textiles, rope, magical components";
  case RESOURCE_STONE:
    return "Building materials, tools";
  case RESOURCE_GAME:
    return "Leather armor, clothing";
  case RESOURCE_WATER:
    return "Alchemy, food preparation";
  case RESOURCE_CLAY:
    return "Pottery, containers, building";
  case RESOURCE_SALT:
    return "Food preservation, alchemy";
  default:
    return "Various crafting applications";
  }
}
...
const char *get_quality_bonus_description(int quality)
{
  ...
  {
  case 1:
    return "poor quality (-25%)";
  case 2:
    return "common quality (+0%)";
  case 3:
    return "uncommon quality (+50%)";
  case 4:
    return "rare quality (+200%)";
  case 5:
    return "legendary quality (+500%)";
  default:
...
void show_basic_material_storage(struct char_data *ch)
{
  ...
  for (i = 0; i < ch->player_specials->saved.stored_material_count; i++)
  {
    ...
    send_to_char(ch, "%s%3d%s x %s\r\n", CCYEL(ch, C_NRM), mat->quantity, CCNRM(ch, C_NRM),
                 full_name);
  }
  ...
  for (cat = 0; cat < NUM_RESOURCE_TYPES; cat++)
  {
    ...
    for (i = 0; i < ch->player_specials->saved.stored_material_count; i++)
    {
      ...
      const char *enhanced_name =
          get_enhanced_material_name(mat->category, mat->subtype, mat->quality);
      int enhanced_id = get_enhanced_wilderness_material_id(mat->category, mat->subtype);
      int crafting_value =
          get_enhanced_material_crafting_value(mat->category, mat->subtype, mat->quality);
      ...
      send_to_char(ch, "  Crafting Value: %d (quality bonus: %s)\r\n\r\n", crafting_value,
                   get_quality_bonus_description(mat->quality));
    }
  }
...
ACMD(do_wilderness_gather)
{
  ...
  if (resource_type < 0 || (resource_type != RESOURCE_HERBS &&
                            resource_type != RESOURCE_VEGETATION && resource_type != RESOURCE_GAME))
  {
...
ACMD(do_wilderness_mine)
{
  ...
  resource_type = parse_resource_type(arg);
  if (resource_type < 0 ||
      (resource_type != RESOURCE_MINERALS && resource_type != RESOURCE_CRYSTAL &&
       resource_type != RESOURCE_STONE && resource_type != RESOURCE_SALT))
  {
...
int attempt_wilderness_harvest(struct char_data *ch, int resource_type)
{
  ...
  {
    const char *terrain_names[] = {"indoors",        "city",
                                   "field",          "forest",
                                   "hills",          "mountain",
                                   "shallow water",  "deep water",
                                   "flying",         "underwater",
                                   "zone start",     "north-south road",
                                   "east-west road", "road intersection",
                                   "desert",         "ocean",
                                   "marshland",      "high mountain",
                                   "planes",         "underdark"};
    const char *terrain_name =
        (sector_type >= 0 && sector_type < 20) ? terrain_names[sector_type] : "unknown terrain";
  ...
  {
    send_to_char(ch,
                 "This area has been over-harvested. The %s resources are too depleted to yield "
                 "anything.\r\n",
                 resource_names[resource_type]);
    return 0;
  }
  ...
  if (added > 0)
  {
    ...
    {
      send_to_char(ch,
                   "\tc(Your conservation score is affected by large-scale harvesting.)\tn\r\n");
    }
