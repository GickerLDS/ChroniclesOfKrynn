...
const char *const mission_targets[MISSION_TARGET_FIELDS] = {
    "person", "traitor", "darkling", "fugitive", "fugitive",
};
...
const char *const target_difficulty[NUM_MISSION_DIFFICULTIES] = {
    "of easy strength",           "of normal strength",        "above normal strength",
    "well above normal strength", "far above normal strength", "far above normal strength",
};

const char *const guard_difficulty[NUM_MISSION_DIFFICULTIES] = {
    "of easy strength",      "of normal strength",    "of normal strength",
    "above normal strength", "above normal strength", "well above normal strength",
};
...
SPECIAL(faction_mission)
{
  ...
  if (is_abbrev(argument, "decline"))
  {
    ...
    {
      send_to_char(ch,
                   "If you wish to confirm this mission decline, type \tYmission decline\tn a "
                   "second time.\r\n"
                   "If you do not wish to decline the mission, type \tYmission accept\tn to clear "
                   "the decline flag.\r\n"
    ...
    send_to_char(ch,
                 "You have turned down the offered mission.  This has reduced your faction "
                 "standing with the %s by 50 points. Your cooldown has been halved.\r\n",
                 faction_names_lwr[GET_MISSION_FACTION(ch)]);
  ...
  {
    send_to_char(
        ch, "You are currently on a mission.  Type 'mission decline' before taking a new one.\r\n");
    return TRUE;
  }
  ...
  snprintf(buf, sizeof(buf),
           ...
           "Take note: this mission is of %s difficulty. Your target will be %s and the guards "
           "will be %s.'\r\n",
           faction_names_lwr[faction], AN(mission_targets[mission_details_to_faction(faction)]),
           ...
           mission_difficulty[difficulty], target_difficulty[difficulty],
           guard_difficulty[difficulty]);
  act(buf, FALSE, ch, 0, (struct char_data *)me, TO_CHAR);

  send_to_char(ch,
               "Your reward will be %ld gold coins, %ld %s standing, %ld quest points and %ld "
               "experience points.\r\n\tn",
...
const char *get_mission_zone_name(struct char_data *ch)
{
  ...
  {
    snprintf(name, sizeof(name), "the %s",
             zone_table[world[GET_CURRENT_MISSION_ROOM(ch)].zone].name);
    return strdup(name);
  }
#else
...
ACMD(do_missions)
{
  ...
  send_to_char(ch,
               ...
               random_npc_names[GET_MISSION_NPC_NAME_NUM(ch)], get_mission_zone_name(ch),
...
bool is_mission_mob(char_data *ch, char_data *mob)
{
  ...
  {
    if (IS_NPC(tch) && tch->master && !IS_NPC(tch->master) && is_player_grouped(ch, tch->master) &&
