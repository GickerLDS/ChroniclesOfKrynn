...
EVENTFUNC(event_brewing)
{
  ...
  if (num_spells == 1)
  {
    send_to_char(ch, "\tGYou finish brewing %s potion%s of %s!\tn\r\n",
                 critical_success ? "two exceptional" : "a", critical_success ? "s" : "",
                 spell_info[spell_nums[0]].name);
  }
  else
  {
    send_to_char(ch, "\tGYou finish brewing %s complex %d-spell potion%s!\tn\r\n",
                 critical_success ? "two exceptional" : "a", num_spells,
                 critical_success ? "s" : "");
...
int get_mote_type_for_school(int school)
{
  ...
  {
  case ABJURATION:
    return CRAFTING_MOTE_LIGHT; /* Protection/warding */
  case CONJURATION:
    return CRAFTING_MOTE_EARTH; /* Creation/summoning */
  case DIVINATION:
    return CRAFTING_MOTE_AIR; /* Knowledge/detection */
  case ENCHANTMENT:
    return CRAFTING_MOTE_LIGHTNING; /* Mind affecting */
  case EVOCATION:
    return CRAFTING_MOTE_FIRE; /* Energy/force */
  case ILLUSION:
    return CRAFTING_MOTE_ICE; /* Deception/shadow */
  case NECROMANCY:
    return CRAFTING_MOTE_DARK; /* Death/negative energy */
  case TRANSMUTATION:
    return CRAFTING_MOTE_WATER; /* Change/alteration */
  default:
    return CRAFTING_MOTE_EARTH; /* Default fallback */
  }
}
...
int get_gold_cost_for_spell(int spell_circle)
{
  ...
  {
  case 0:
    return 25; /* Cantrips/orisons */
  case 1:
    return 50; /* 1st level */
  case 2:
    return 100; /* 2nd level */
  case 3:
    return 200; /* 3rd level */
  case 4:
    return 400; /* 4th level */
  case 5:
    return 600; /* 5th level */
  case 6:
    return 900; /* 6th level */
  case 7:
    return 1200; /* 7th level */
  case 8:
    return 1600; /* 8th level */
  case 9:
    return 2000; /* 9th level */
  case 10:
    return 5000; /* Epic spells */
  default:
    return 25;
  }
}
...
bool has_brew_materials(struct char_data *ch, int spell_num)
{
  ...
  if (GET_CRAFT_MOTES(ch, mote_type) < motes_required)
  {
    ...
    send_to_char(ch, "You need %d %s%s to brew this potion (you have %d).\r\n", motes_required,
                 mote_name, motes_required > 1 ? "s" : "", GET_CRAFT_MOTES(ch, mote_type));
    return FALSE;
  }
...
struct obj_data *create_potion(int spell_num, struct char_data *ch)
{
  ...
  snprintf(long_desc, sizeof(long_desc),
           "A small vial containing a magical potion of %s lies here.", spell_info[spell_num].name);
...
struct obj_data *create_multi_spell_potion(int *spell_nums, int num_spells, struct char_data *ch)
{
  ...
  if (num_spells == 1)
  {
    ...
    snprintf(long_desc, sizeof(long_desc),
             "A small vial containing a magical potion of %s lies here.",
             spell_info[spell_nums[0]].name);
  }
  else
  {
    ...
    snprintf(long_desc, sizeof(long_desc),
             "A small vial containing a complex magical potion with %d spells lies here.",
             num_spells);
  }
...
ACMD(do_brew)
{
  ...
  for (i = 0; i < NUM_CRAFT_MOTES; i++)
  {
    ...
    {
      send_to_char(ch, "You need %d %s%s but only have %d.\r\n", total_motes_by_type[i],
                   crafting_motes[i], total_motes_by_type[i] > 1 ? "s" : "",
